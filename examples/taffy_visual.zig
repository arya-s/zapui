//! Taffy Visual Demo
//!
//! Renders a dashboard layout using Taffy for flexbox and zapui for rendering.
...
pub fn main() !void {
    ...
    const window = glfw.glfwCreateWindow(@intFromFloat(WINDOW_WIDTH), @intFromFloat(WINDOW_HEIGHT), "Taffy Layout Demo", null, null);
    if (window == null) {
    ...
    // Build Taffy layout tree - Simple 2x3 grid layout
    ...
    const root = try tree.newLeaf(.{
        ...
        .padding = taffy.Rect(taffy.LengthPercentage).all(.{ .length = padding }),
    ...
    // 5 sidebar items
    var sidebar_items: [5]taffy.NodeId = undefined;
    for (0..5) |i| {
    ...
    var top_cards: [3]taffy.NodeId = undefined;
    for (0..3) |i| {
    ...
    var bottom_cards: [3]taffy.NodeId = undefined;
    for (0..3) |i| {
    ...
    std.debug.print("Taffy Visual Demo\n", .{});
