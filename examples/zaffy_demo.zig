//! Zaffy Layout Demo
//!
//! This example demonstrates using Zaffy for automatic flexbox layout.
...
pub fn main() !void {
    ...
    std.debug.print("Zaffy Layout Demo\n", .{});
    std.debug.print("=================\n\n", .{});

    // Build layout tree with Zaffy
    ...
    const root = try tree.newLeaf(.{
        ...
        .padding = zaffy.Rect(zaffy.LengthPercentage).all(.{ .length = 20 }),
    ...
    const header = try tree.newLeaf(.{
        ...
        .align_items = .center,
        .padding = zaffy.Rect(zaffy.LengthPercentage).all(.{ .length = 16 }),
    });
    ...
    // Nav items
    var nav_items: [4]zaffy.NodeId = undefined;
    for (0..4) |i| {
