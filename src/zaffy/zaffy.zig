//! ZaffyTree - High-level layout tree API
//!
...
pub fn ZaffyTree(comptime NodeContext: type) type {
    return struct {
        const Self = @This();
...
/// Default ZaffyTree with no node context
pub const Zaffy = ZaffyTree(void);

// ============================================================================
// Tests
// ============================================================================

test "ZaffyTree basic" {
    var tree = Zaffy.init(std.testing.allocator);
    defer tree.deinit();
...
test "ZaffyTree flex grow ratio" {
    var tree = Zaffy.init(std.testing.allocator);
    defer tree.deinit();
...
test "ZaffyTree padding and gap" {
    var tree = Zaffy.init(std.testing.allocator);
    defer tree.deinit();
...
test "ZaffyTree justify content" {
    var tree = Zaffy.init(std.testing.allocator);
    defer tree.deinit();
